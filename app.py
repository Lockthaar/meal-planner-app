...
if st.session_state.onboard_step == 0:
    ...
    solo_col, couple_col, fam_col = st.columns(3)
    with solo_col:
...
if st.session_state.onboard_step == 1:
    ...
    st.write("## 2. Combien de repas par jour souhaitez-vous ?")
    two, three, four = st.columns(3)
    with two:
...
if page == "Accueil":
    ...
    st.write("Utilisez la navigation ci-dessus pour ajouter recettes, extras, planifier la semaine…")
...
# --------------------------------------------------
    ...
    with left.form("add_recipe"):
        ...
        for i in range(n):
            nm = cols[0].text_input(f"Nom #{i+1}", key=f"ing_nm_{i}")
        ...
        img   = st.text_input("URL image", "https://via.placeholder.com/150")
        ...
        for chunk in (days[:3], days[3:6], days[6:]):
            ...
            for col, day in zip(cols, chunk + [""]*(3-len(chunk))):
                if not day:
                    ...
                    continue
                col.markdown(
                    f"<div style='border:1px solid #444; border-radius:8px; padding:8px;'><strong>{day}</strong></div>",
